...
var (
	route_push      = regexp.MustCompile(`push$`)
	route_pushs     = regexp.MustCompile(`pushs$`)
	route_pushall   = regexp.MustCompile(`all$`)
	route_pushroom  = regexp.MustCompile(`room$`)
	route_serverdel = regexp.MustCompile(`del$`)
	route_count     = regexp.MustCompile(`count$`)
)

func route(w http.ResponseWriter, r *http.Request) {
	switch {
	case route_push.MatchString(r.URL.Path):
		Push(w, r)
	case route_pushs.MatchString(r.URL.Path):
		Pushs(w, r)
	case route_pushall.MatchString(r.URL.Path):
		PushAll(w, r)
	case route_pushroom.MatchString(r.URL.Path):
		PushRoom(w, r)
	case route_serverdel.MatchString(r.URL.Path):
		DelServer(w, r)
	case route_count.MatchString(r.URL.Path):
		Count(w, r)
	default:
		fmt.Println("没有此路由")
	}
